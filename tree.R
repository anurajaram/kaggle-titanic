...
train = read.delim(file = 'train.csv',  header = TRUE, 
                  sep = ',', dec = '.')
# adding the stringasFactors() worked for the airbnb kaggle dataset, 
# but not this one!
...
# data explore
summary(train)
sapply(train, function(x) sum(is.na(x)))  # this shows the number of NAs for each variable in the dataset
